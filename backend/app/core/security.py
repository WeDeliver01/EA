...
def hash_refresh_token(token: str) -> str:
    """Same rationale as `hash_api_key`: a refresh token is a high-entropy
    opaque secret generated by us (`secrets.token_urlsafe`), not a human
    password, so a fast deterministic hash that supports lookup-by-hash is
    correct - matches `refresh_tokens.token_hash`'s unique-index design."""
